...
class PersonRepository:
    ...
    def update(self, guid, profile):
        ...
        try:
            ...
            abort(503, f'Record could not be updated. Message: {e}')


    def remove(self, guid):
        """ Remove a person by its globally unique identifier (GUID) """
        
        logger.info(f'Removing individual information:"{guid}"')
        
        if self.get(guid) == None:
            abort(404, f'No existing record existing record has been found for GUID {guid}.')
        
        try:
            cursor = rdb.table(args.rethinkdb_table).filter({args.rethinkdb_primary_key: guid}).delete().run(g.rdb_conn)
            return cursor
        except RqlRuntimeError as e:
            abort(503, f'Record could not be removed. Message: {e}')
